...
-- Problem: omsaetning_net should be BRUTTO revenue (without discounts subtracted)
-- Current: revenue_gross - order_discounts - cancelled_amount - refunded_amount
-- Correct: revenue_gross - cancelled_amount - refunded_amount (NO DISCOUNTS!)
--
-- Why: Discounts should be tracked separately and subtracted in Google Sheets
--
-- Example (2025-10-01):
-- Expected: 948.10 (brutto without discounts)
-- Actual: 831.89 (incorrectly subtracting discounts)
...
sales AS (
  SELECT
    ...
    sb.artikelnummer,
    -- Quantities (EXCLUDE cancelled - brutto should be sold items only)
    ...
    -- cancelled_amount_dkk is already the total amount - DO NOT multiply!
    SUM(COALESCE(sb.cancelled_amount_dkk, 0))::numeric(14,2) AS cancelled_amount,
    -- Track discounts separately (for reference, NOT subtracted from omsaetning_net)
    ...
    SUM(sb.total_discount_dkk)::numeric(14,2) AS total_discounts
  FROM sales_base sb
...
joined AS (
  SELECT
    ...
    -- ✅ FIX: omsaetning_net = revenue_gross - cancelled_amount - refunded_amount
    -- NO DISCOUNTS SUBTRACTED (discounts handled in Google Sheets)
    (COALESCE(s.revenue_gross, 0)
     - COALESCE(s.cancelled_amount, 0)
     - COALESCE(r.refunded_amount, 0))::numeric(10,2) AS omsaetning_net,
    ...
    COALESCE(s.total_discounts, 0)::numeric(10,2) AS total_discounts,  -- Track for reference
...
-- Verify result for 2025-10-01
SELECT
  'Verification for 2025-10-01' AS description,
...
-- Check specific artikelnummer 100145
SELECT
  metric_date,
  artikelnummer,
  solgt,
