...
# Create all shipping discount jobs for historical sync
# Usage: ./create-all-shipping-discount-jobs.sh

KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImloYXdqcnRmd3lzeW9rZm90ZXduIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1ODA0OTMyOCwiZXhwIjoyMDczNjI1MzI4fQ.MzRIK7zmo-O8yt89vxYsw9DVMLyHLo7OUSLSnXaOUJM"

echo "🚀 Creating all shipping discount jobs..."

while true; do
  result=$(curl -s -X POST "https://ihawjrtfwysyokfotewn.supabase.co/functions/v1/create-missing-jobs" \
    -H "Authorization: Bearer $KEY" \
    -H "Content-Type: application/json" \
    -d '{"startDate": "2024-09-30", "endDate": "2025-10-12", "objectType": "shipping-discounts"}')

  echo "$result"

  if echo "$result" | grep -q '"complete":true'; then
    echo "✅ All shipping discount jobs created!"
    break
  fi
...
curl -s "https://ihawjrtfwysyokfotewn.supabase.co/rest/v1/bulk_sync_jobs?object_type=eq.shipping-discounts&select=status" \
  -H "apikey: $KEY" \
  -H "Authorization: Bearer $KEY" \
