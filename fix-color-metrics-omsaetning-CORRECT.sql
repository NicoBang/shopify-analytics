...
-- Problem: omsaetning_net is currently subtracting refunded_amount, but it should NOT
--
-- CORRECT formula:
-- omsaetning_net = revenue_gross - total_discount_dkk - cancelled_amount
-- (refunded_amount is stored separately and subtracted in Google Sheets)
--
-- Example (2025-10-01, artikelnummer 100145):
-- Sales: 948.10 (after subtracting total_discount_dkk)
-- Refunds: 116.21 (stored separately, NOT subtracted here)
-- Expected omsaetning_net: 948.10
-- Current (wrong): 831.89 (incorrectly subtracting refunded_amount)
...
sales AS (
  SELECT
    ...
    -- Store cancelled_amount separately for reference
...
joined AS (
  SELECT
    ...
    -- ✅ CORRECT: omsaetning_net = revenue_net (already has discounts subtracted)
    -- DO NOT subtract refunded_amount here! (Google Sheets does that)
...
SELECT
  'Artikelnummer 100145 for 2025-10-01' AS description,
  metric_date,
  artikelnummer,
  solgt,
  retur,
