...
-- Problem 1: Missing sale_discounts in omsaetning_net calculation
-- Problem 2: cancelled_amount is being multiplied by cancelled_qty (wrong!)
--
-- Correct formula (matching daily_shop_metrics):
-- omsaetning_net = revenue_gross - order_discounts - sale_discounts - cancelled_amount - refunded_amount
--
-- Where:
-- - revenue_gross = price_dkk * quantity (includes cancelled items)
-- - cancelled_amount = cancelled_amount_dkk (DO NOT multiply by cancelled_qty!)
-- - order_discounts = discount_per_unit_dkk * quantity
-- - sale_discounts = sale_discount_total_dkk (already calculated)
...
sales AS (
  SELECT
    ...
    -- ✅ FIX: cancelled_amount_dkk is already the total amount - DO NOT multiply!
...
joined AS (
  SELECT
    ...
    -- ✅ FIX: Include BOTH order_discounts AND sale_discounts
    -- omsaetning_net = revenue_gross - order_discounts - sale_discounts - cancelled_amount - refunded_amount
    (COALESCE(s.revenue_gross, 0)
     - COALESCE(s.order_discounts, 0)
     - COALESCE(s.sale_discounts, 0)  -- ✅ ADDED
